...
<html>
	...
	<body>
		<a href="#list-capture" class="skip" tabindex="-1"><g:message code="default.link.skip.label" default="Skip to content&hellip;"/></a>
		...
		<div id="list-capture" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="captureDate" title="${message(code: 'capture.captureDate.label', default: 'Capture Date')}" />
				...
				<g:each in="${captureInstanceList}" status="i" var="captureInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${captureInstance.id}">${fieldValue(bean: captureInstance, field: "line")}</g:link></td>
					
						<td>${fieldValue(bean: captureInstance, field: "population")}</td>
