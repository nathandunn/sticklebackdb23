...
<body>
  ...
  <div id="list-stock" class="content scaffold-list" role="main">
    ...
    <table>
      <thead>
        <tr>
      <g:sortableColumn property="stockID" title="${message(code: 'stock.abbreviation.label', default: 'ID')}" />
      <g:sortableColumn property="stockName" title="${message(code: 'stock.abbreviation.label', default: 'Stock Name')}" />
      </tr>
      </thead>
      <tbody>
      <g:each in="${stockInstanceList}" status="i" var="stockInstance">
        <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
          <td><g:link action="show" id="${stockInstance.id}" controller="stock">
          <g:formatNumber number="${stockInstance?.stockID}" minFractionDigits="4"/>
        </g:link></td>
